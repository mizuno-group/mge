...
function pair_end () {
  ...
  if "${compress}"; then
    for ix in ${!q1[@]}; do
      ...
      temp2=${temp/".${extension}"/''}
      echo $temp2
      prinseq++ -out_gz -fastq ${q1[ix]} -fastq2 ${q2[ix]} -out_name ${curr_par}/tmp_dir/${temp2} -threads ${threads} \
        -trim_left 5 -trim_tail_right 5 -trim_qual_right 30 -ns_max_n 20 -min_len 30
    done
    mkdir -p ${curr_par}/res_prinseq
    for g1 in ${curr_par}/tmp_dir/*.fastq.gz; do
      if [[ ${g1} =~ "good_out" ]]; then
        mv ${g1} ${curr_par}/res_prinseq
      elif [[ ${g1} =~ "bad_out" ]] || [[ ${g1} =~ "single_out" ]]; then
        rm ${g1}
...
flag=0
compress=false
while getopts m:e:t:g:hv opt; do
  case "$opt" in
    m)
      if [ $OPTARG -eq 1 ]; then
        flag=1
