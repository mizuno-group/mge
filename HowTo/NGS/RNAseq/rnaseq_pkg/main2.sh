...
if [ ${SEQ_END} = "single" ]; then
  docker compose exec prinseq /main.sh -e ${EXTENSION} -m 1 -g ${RES_ONLY} ${cont_input}
elif [ ${SEQ_END} = "pair" ]; then
  docker compose exec prinseq /main.sh -e ${EXTENSION} -m 2 -g ${RES_ONLY} ${cont_input}
else
...
if [ -z "${SALMON_IDX_PATH}" ]; then
  docker compose exec salmon /prep_index.sh ${cont_input} ${salmon_tx} ${salmon_gen}
  SALMON_IDX_PATH=`find ${host_par}/salmon_indices -maxdepth 1 -type d -name "salmon_index_*"`
  if [ -z "${SALMON_IDX_PATH}" ]; then
...
if [ ${SEQ_END} = "single" ]; then
  docker compose exec salmon /main.sh -m 1 -d ${RES_ONLY} ${cont_par}/res_prinseq ${cont_idx}
elif [ ${SEQ_END} = "pair" ]; then
  docker compose exec salmon /main.sh -m 2 -d ${RES_ONLY} ${cont_par}/res_prinseq ${cont_idx}
else
  echo "!! ERROR: SEQ_END should be single or pair !!"
  exit 1
fi
...
if [ -z "${TX_GTF_PATH}" ]; then
  docker compose exec tximport Rscript /main.R ${cont_par}/res_salmon ${url_gtf}
else
  cont_gtf=`echo ${TX_GTF_PATH//${BSRC}/"/workspace"}`
  docker compose exec tximport Rscript /main.R ${cont_par}/res_salmon ${url_gtf} ${cont_gtf}
fi
echo ">> tximport status: $?"

# summary
docker compose exec salmon python /integrate.py -i ${cont_par}/res_gene -o ${OUTPUT_EXP}
docker compose exec salmon python /integrate.py -i ${cont_par}/res_tx -o ${OUTPUT_TX}
docker compose exec salmon mkdir ${cont_par}/res_summary
docker compose exec salmon mv ${cont_par}/res_gene/${OUTPUT_EXP}.txt ${cont_par}/res_tx/${OUTPUT_TX}.txt ${cont_par}/res_summary/.
...
if "${RES_ONLY}"; then
  docker compose exec salmon mv -T ${cont_par}/res_summary ${res_path}
  docker compose exec salmon rm -rf ${cont_par}/res_prinseq \
    ${cont_par}/res_salmon \
    ${cont_par}/res_gene \
    ${cont_par}/res_tx
else
